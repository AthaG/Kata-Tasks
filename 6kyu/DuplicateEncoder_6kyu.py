...
string, or ")" if that character appears more than once in the original string. Ignore
...
def duplicate_encode(word):
    word = word.lower()
    counts = Counter(word)
    return ''.join(')' if counts[i] > 1 else '(' for i in word)

#Second solution
def duplicate_encode(word):
    word = word.lower()
    wordres = [')' if word.count(i) > 1 else '(' for i in word]
    return ''.join(wordres)
