'''A traveling salesman has to visit clients. He got each client's address e.g. "432 
Main Long Road St. Louisville OH 43071" as a list.
...
Task:
The function travel will take two parameters r (addresses' list of all clients' as a 
string) and zipcode and returns a string in the following format:
...
def travel(r, zipcode):
    ...
    for i in splitL:
        if zipcode[-5:] == i[-5:]:
            result.append(str(i[:-9]))
    first = ','.join([i.split(' ')[0] for i in result])
    final = ','.join([' '.join(v) for v in [i.split(' ')[1:] for i in result]])

    return (f'{zipcode}:{final}/{first}')
