...
Notes:
    ...
    In return, you may safely assume that all input strings passed into your function(s) will not contain redundant leading/trailing whitespace, i.e. you do not and should not trim the input string before operating on it
...
def code(s):
    n = ceil(sqrt(len(s)))
    s = s.ljust(n**2, chr(11))
    row = [s[n*k: n*(k+1)] for k in range(n)]
    return '\n'.join(''.join(row[c][i] for c in range(n)[::-1]) for i in range(n))

def decode(s):
    row = [s[::-1] for s in s.split('\n')]
    return ''.join(''.join(row[c][i] for c in range(len(row[0]))) for i in range(len(row[0]))).replace(chr(11), '')
